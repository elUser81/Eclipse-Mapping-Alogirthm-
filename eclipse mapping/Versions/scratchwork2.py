...
class Binary:
    ...
    def get_synthetic_fluxes(self):
        ...
        def synth_flux(I, phi):
            ...
            return flux[0][0] #Why is that happening I have no idea what is going on????? an embedded array?  why python
    ...
    def build_chi_constraint(self):
        ...
        def chi_constraint(I):
            ...
            def synth_flux(I, phi):
                index = int(where(phase_points == phi))
                V_phi = flattened_matrix[:, index]
                I_times_V = [i * v for i, v in zip(I, V_phi)]
    ...
    def get_image_solution(self):
        ...
        res = minimize(S, x0=guess, method='SLSQP')
        stop = time.time()
