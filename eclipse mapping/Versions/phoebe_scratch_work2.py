...
class Binary:
    ...
    def build_visibility_matrix(self):
        ...
        for i in tqdm(range(len(self.constructed_phasepoints)), 'Building Visibility Matrix: '):
    ...
    def build_chi_constraint(self):
        ...
        def chi_constraint(I):
            ...
            if chi_sqrd == inf or chi_sqrd == nan:
                chi_sqrd = nan_to_num(chi_sqrd)
    ...
    def get_image_solution(self):
        ...
        res = minimize(S, x0=guess, method='SLSQP', constraints=cons)
        stop = time.time()
...
class Processes:
    ...
    def get_params_from_file(params_path):
        ...
        to_float = ['ecc', 'chi_AIM', 'period', 'solve time', 'distance', 'incl',
                    'radius_primary', 'q', 'radius_secondary', 'sma', 'delta',
                    'mass_primary', 'mass_secondary', 'temperature_primary', 'temperature_secondary', 'phi0']
...
cwd = getcwd()
#binary_dir = join(cwd, 'Outputs', 'Results', 'KIC_201325107(15)')
...
plt.plot(data[:,1], 'bx')
plt.plot(data[peaks]'yx')
'''
'''
save_path = join(cwd, 'data', 'KIC_201325107_epochs_21_37_to_33_28.csv')
data_dir = join(cwd, 'data', 'KIC_201325107_epochs_primary_minima')


start_file = 'epoch_at_21.371629000000212.csv'
paths = Processes.get_data_paths(data_dir, start_file, range = 16)

Processes.merge_save_epochs(paths, save_path)
Processes.plot_peaks(save_path, 2000, file_name= 'PLOTTED_PEAKS.png')
'''



binary_dir = join(cwd, 'data', 'Systems', 'EPIC_201325107_epochs_23_37_to_33_28')
b = Processes.load_binary(binary_dir)
