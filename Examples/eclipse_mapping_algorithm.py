...
class Binary:
    ...
    def get_synthetic_fluxes(self):
        ...
        from numpy import cos, asarray, where
        theta_sqrd = ((self.RL1_from_primary / self.distance) ** 2) * cos(self.incl)
...
class Processes:
    ...
    @classmethod
    def save(self, binary_object, save_dir = join(getcwd(), 'Outputs', 'Results')):
        from os.path import join, isdir
        ...
        :return: creates a save file subdirectory within save_dir
        '''

        binary_dir = join(save_dir, binary_object.system_name)
    ...
    @classmethod
    def convolve(self, base, signal, save_dir):
        '''
        :param base: binary object which has the base image solution, should be very smooth and guassian looking
        :param signal: binary object which has the signal image solution, should be rougher, smaller delta value, 1-2
        :return: a binary object whose image solution is the convolution of the two signals. Then I can do analysis
                on it like any other binary object
        '''
        from scipy.signal import convolve2d
        from os.path import join

        base_image, signal_iamge = base.image_solution, signal.image_solution

        convolved = convolve2d(base_image, signal_iamge, mode = 'same')

        new_binary = Binary(base.all_params, base.light_curve_data)
